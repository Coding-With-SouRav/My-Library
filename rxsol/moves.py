...
class PruningTable:
    """
    Helper class to allow pruning to be used as though they were 2-D moves
    """
...
class Tables:
    """
    Class for holding move and pruning moves in memory.

    Move moves are used for updating coordinate representation of cube when a
    particular move is applied.

    Pruning moves are used to obtain lower bounds for the number of moves
    ...
    def load_moves(cls):
        if os.path.isfile("moves.json"):
            ...
            cls.udslice_twist_prune = PruningTable(
                moves["udslice_twist_prune"], cls.TWIST
            )
            cls.udslice_flip_prune = PruningTable(
                moves["udslice_flip_prune"], cls.FLIP
            )
            cls.edge4_edge8_prune = PruningTable(
                moves["edge4_edge8_prune"], cls.EDGE8
            )
            cls.edge4_corner_prune = PruningTable(
                moves["edge4_corner_prune"], cls.CORNER
            )
        else:
            # ----------  Phase 1 move moves  ---------- #
