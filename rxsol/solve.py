...
class SolutionManager:
    ...
    def _phase_1_initialise(self, max_length):
        ...
        # after n moves. these estimates come from the pruning moves and are
        ...
        if time.time() > self._timeout:
            ...
            for i in range(6):
                ...
                for j in range(1, 4):
                    ...
                    # update coordinates
                    self.twist[n + 1] = self.moves.twist_move[self.twist[n]][
                        mv
                    ]
                    self.flip[n + 1] = self.moves.flip_move[self.flip[n]][mv]
                    self.udslice[n + 1] = self.moves.udslice_move[
                        self.udslice[n]
                    ][mv]
        ...
        if self.min_dist_2[n] == 0:
            ...
            for i in range(6):
                ...
                for j in range(1, 4):
                    ...
                    self.edge4[n + 1] = self.moves.edge4_move[self.edge4[n]][
                        mv
                    ]
                    self.edge8[n + 1] = self.moves.edge8_move[self.edge8[n]][
                        mv
                    ]
                    self.corner[n + 1] = self.moves.corner_move[
                        self.corner[n]
                    ][mv]
